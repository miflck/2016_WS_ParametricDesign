...
ArrayList<PVector> vokalpositions;
...
void setup() {
  ...
  vokalpositions= new ArrayList<PVector>();
...
void computePositions() {
  ...
  for (int i=0; i<data.length; i++) { // Alle Zeilen durchgehen
    for (int k=0; k<data[i].length(); k++) { // Alle Buchstaben der Zeile durchgehen
      ...
      for (int l=0; l<vokale.length; l++) {
        if (c==vokale[l]) {
          println(c+" "+vokale[l]);
          vokalpositions.add(p);
        }
      }
    }
  }
...
void draw() {
  ...
  for (int i=0; i<vokalpositions.size(); i++) {
    PVector p=vokalpositions.get(i);
